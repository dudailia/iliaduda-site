 ...
 * histogram, filling the same world rectangle the live camera opens on, so
 * the crossfade lands on the same picture. Strokes do not scale; labels are
...
function Label({ x, y, cls, children }: { x: number; y: number; cls: string; children: ReactNode }) {
  return (
    <span className={`${LABEL} ${cls}`} style={{ left: `${((x / VB.w) * 100).toFixed(2)}%`, top: `${((y / VB.h) * 100).toFixed(2)}%` }}>
      {children}
    </span>
  )
}

export function Poster({ strands, bars, strike }: { strands: Strand[]; bars: Bar[]; strike: number }) {
  ...
  return (
    <>
      <svg viewBox={`0 0 ${VB.w} ${VB.h}`} preserveAspectRatio="none" className="absolute inset-0 size-full" aria-hidden="true">
        ...
        <g>
          {bars.map((b, i) => (
            <rect key={i} x={b.x} y={b.y} width={b.w} height={b.h} className={b.pays ? 'fill-indigo' : 'fill-graphite'} fillOpacity={b.pays ? 0.5 : 0.28} />
          ))}
        </g>
      ...
      <div aria-hidden="true">
        ...
        <Label x={w(LABELS.hist.at)[0]} y={w(LABELS.hist.at)[1]} cls={`${LABELS.hist.cls} text-graphite`}>
          Where the futures end
        </Label>
      </div>
    </>
  )
