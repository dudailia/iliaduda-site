...
for p in $ROUTES; do for i in $(seq 1 $RUNS); do
  ...
  node -e 'const r=require(process.argv[1]);const c=r.categories,a=r.audits;console.log(process.argv[1].split("/").pop(),"perf",Math.round(c.performance.score*100),"a11y",Math.round(c.accessibility.score*100),"LCP",Math.round(a["largest-contentful-paint"].numericValue),"TBT",Math.round(a["total-blocking-time"].numericValue),"CLS",a["cumulative-layout-shift"].numericValue.toFixed(4),"bytes",Math.round(a["total-byte-weight"].numericValue))' "$OUT/m$n-$i.json"
done; done
